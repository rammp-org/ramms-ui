...
void URammsSimLayout::BuildWidgetTree()
{
	...
	{
		UOverlay* Slot = WidgetTree->ConstructWidget<UOverlay>(UOverlay::StaticClass(), TEXT("SurfacePanel"));
		if (UCanvasPanelSlot* CanvasSlot = Root->AddChildToCanvas(Slot))
		{
		...
		RegisterSlot(TEXT("SurfacePanel"), Slot);
	}
	// Bottom-left: the drive joystick.
	{
		UOverlay* Slot = WidgetTree->ConstructWidget<UOverlay>(UOverlay::StaticClass(), TEXT("Joystick"));
		if (UCanvasPanelSlot* CanvasSlot = Root->AddChildToCanvas(Slot))
		{
		...
		RegisterSlot(TEXT("Joystick"), Slot);
	}
	// Top-left: status.
	{
		UOverlay* Slot = WidgetTree->ConstructWidget<UOverlay>(UOverlay::StaticClass(), TEXT("Status"));
		if (UCanvasPanelSlot* CanvasSlot = Root->AddChildToCanvas(Slot))
		{
